...
scorecard_model = pd.read_pickle("modelo_scorecard3.pkl")
...
def score():
    ...
    with st.container():
        ...
        person_income = st.number_input("Ingreso Personal anual (valores en USD)", min_value=0.0)
        ...
        person_emp_length = st.number_input("Experiencia Laboral (en años)", min_value=0.0,max_value=100)
        # Lista desplegable con etiquetas comprensibles
        loan_intent = st.selectbox("Propósito del Préstamo", list(intent_labels.values()))
        cb_person_default_on_file = st.selectbox("¿Has incumplido alguna vez con un credito?", list(cb_person_default_on_file_labels.values()))
        cb_person_cred_hist_length = st.number_input("Historial credicticio (en años)", min_value=0.0, max_value=100)
